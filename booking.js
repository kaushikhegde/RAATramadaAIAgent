...
const BROWSER_CHANNEL = process.env.BROWSER_CHANNEL || ""; // e.g. "chrome" launches system Chrome
const PROFILE_DIR = process.env.PROFILE_DIR || path.join(__dirname, ".jetstar-profile");
...
function buildJetstarUrl(booking) {
  if (!booking.originCode || !booking.destinationCode) {
    throw new Error(
      `Missing airport code(s): originCode=${booking.originCode}, destinationCode=${booking.destinationCode}. ` +
      `Booking source must populate IATA codes before calling runJetstarBooking.`
    );
  }
...
async function step17_finalActions(page, pdfFileName) {
  ...
  // Move PDF from Todo/ to Completed/ (chat flow has no PDF — skip)
  if (pdfFileName) {
...
async function buildItinerary(page, bookingInput) {
  const url = page.url();
  let title = null;
  let pageText = "";
  try { title = await page.title(); } catch { /* ignore */ }
  try { pageText = await page.evaluate(() => document.body.innerText); } catch { /* ignore */ }

  // Best-effort regex scrape — selectors break easily, so we keep this defensive.
  const flightNumbers = Array.from(new Set((pageText.match(/\bJQ\s?\d{2,4}\b/gi) || []).map(s => s.replace(/\s+/g, "").toUpperCase())));
  const totalMatch = pageText.match(/(?:Total|Grand total)[^\d]{0,40}([A-Z]{0,3}\s?\$?\s?[\d,]+\.\d{2})/i);
  ...
  return {
    ...
    booking: {
      ...
      originCode: bookingInput.originCode,
      destination: bookingInput.destination,
      destinationCode: bookingInput.destinationCode,
      tripType: bookingInput.tripType,
      departureDate: bookingInput.departureDate,
      returnDate: bookingInput.returnDate || null,
      adults: bookingInput.adults,
      children: bookingInput.children,
      childAges: bookingInput.childAges || [],
      infants: bookingInput.infants,
      infantAges: bookingInput.infantAges || [],
      budget: bookingInput.budget || null,
      budgetCurrency: bookingInput.budgetCurrency || null,
    },
    detected: {
      flightNumbers,
...
function saveItineraryJson(itinerary) {
  if (!fs.existsSync(COMPLETED_DIR)) fs.mkdirSync(COMPLETED_DIR, { recursive: true });
  const ts = new Date().toISOString().replace(/[:.]/g, "-");
  const fname = `itinerary-${ts}.json`;
  const fullPath = path.join(COMPLETED_DIR, fname);
  fs.writeFileSync(fullPath, JSON.stringify(itinerary, null, 2));
  return fullPath;
}

// ═══════════════════════════════════════════════════════════════════
//  Core automation — exported for both CLI and chat callers
// ═══════════════════════════════════════════════════════════════════
//
// Caller-supplied callbacks (all optional):
//   onStep(stepIdx, message)     — progress; stepIdx maps to the 8-step chat card
//   onError(message)             — surfaces an error message
//   onComplete(message)          — final success message
//   onItinerary(itinerary, path) — itinerary JSON + saved file path
//
// Chat mode (called from server.js):  pdfFileName=null, callbacks supplied.
//   Browser must already be running on CDP_PORT (start-chrome.sh).
// CLI mode (called from the IIFE):    pdfFileName set, callbacks empty.
//   Falls back to launchPersistentContext if USE_PLAYWRIGHT=true.

async function runJetstarBooking({ booking, pdfFileName = null, callbacks = {} } = {}) {
  const onStep      = callbacks.onStep      || (() => {});
  const onError     = callbacks.onError     || (() => {});
  const onComplete  = callbacks.onComplete  || (() => {});
  const onItinerary = callbacks.onItinerary || (() => {});
  const notify = (stepIdx, msg) => { console.log(`[automation step ${stepIdx}] ${msg}`); onStep(stepIdx, msg); };

  let browser, context, page;
  let usingCDP = false;

  // Chat callers don't carry a PDF — if no pdfFileName is provided, force CDP-only
  // and skip the persistent-context fallback (server assumes start:chrome is running).
  const cdpOnly = !pdfFileName;

  if (!USE_PLAYWRIGHT || cdpOnly) {
    ...
    // ── FALLBACK: Persistent Playwright-managed browser ──
    // Uses launchPersistentContext so cookies, storage, and Akamai/Jetstar
    // bot-bypass tokens carry over between runs. The first run may bounce
    // to "Oops" — manually browse jetstar.com in the opened window once,
    // close, and re-run. With BROWSER_CHANNEL="chrome" Playwright drives
    // the installed system Chrome binary against this dedicated profile.
    const profileExists = fs.existsSync(PROFILE_DIR) && fs.readdirSync(PROFILE_DIR).length > 0;
    if (BROWSER_CHANNEL) {
      console.log(`🚀 Launching system Chrome (channel: ${BROWSER_CHANNEL}) with persistent profile...`);
    } else {
      console.log("🚀 Launching Playwright Chromium with persistent profile...");
    }
    console.log(`   Profile: ${PROFILE_DIR} ${profileExists ? "(reusing existing)" : "(NEW — first run; cookies will be cold)"}\n`);
    ...
    // launchPersistentContext returns a context with one open page already
    page = context.pages()[0] || (await context.newPage());
  }

  try {
    // ── Phase 1: Home Page (www.jetstar.com) ─────────────
    notify(0, "Navigating to Jetstar...");
    await step1_navigate(page, booking);
    await step2_verifyOrigin(page, booking);
    await step3_verifyDestination(page, booking);
    await step4_selectDates(page, booking);
    await step5_verifyPassengers(page, booking);

    notify(1, "Searching for flights...");
    await step6_searchFlights(page);

    // ── Phase 2: Flight Selection (booking.jetstar.com) ──
    notify(2, "Selecting departure flight...");
    await step7_selectDepartureFlight(page);

    notify(3, booking.tripType === "return" ? "Selecting return flight..." : "Skipping return (one-way)...");
    await step8_selectReturnFlight(page, booking.tripType);

    // ── Phase 3: Baggage → Seats → Extras ────────────────
    notify(4, "Selecting baggage...");
    await step9_baggage(page);

    notify(5, "Handling seats...");
    await step10_seats(page, booking.tripType);

    notify(6, "Skipping extras...");
    await step11_extras(page);

    notify(7, "Reaching booking details...");
    await step12_continueToBookingDetails(page);

    // Stopped at Booking Details — passenger form left blank for manual entry.
    log(13, "🛑 Stopped at Booking Details page — passenger form left empty for manual entry.");

    // ── Build & save itinerary ───────────────────────────
    const itinerary = await buildItinerary(page, booking);
    const itineraryPath = saveItineraryJson(itinerary);
    log(0, `📝 Itinerary saved: ${path.relative(__dirname, itineraryPath)}`);
    onItinerary(itinerary, itineraryPath);

    // ── Final screenshot (and PDF move when running from CLI) ──
    await step17_finalActions(page, pdfFileName);

    const completionMsg = "Reached booking details. Itinerary saved.";
    ...
    if (usingCDP) {
      // When connected via CDP, just close our tab — don't kill the shared Chrome.
      try { await page.close(); } catch { /* tab may already be closed */ }
      try { await browser.close(); } catch { /* CDP disconnect */ }
      console.log("\nTab closed. Your Chrome browser remains open. Done.\n");
    } else {
      // Persistent context (CLI fallback): close the context.
      try { await context.close(); } catch { /* already closed */ }
      console.log("\nBrowser closed. Profile saved to: " + PROFILE_DIR + "\n");
    }
  }
}

module.exports = { runJetstarBooking };

// ═══════════════════════════════════════════════════════════════════
//  CLI launcher — only when invoked as `node booking.js` (not require()d)
// ═══════════════════════════════════════════════════════════════════
if (require.main === module) {
  (async () => {
    ...
    try {
      await runJetstarBooking({ booking, pdfFileName: pdf.fileName });
    } catch {
      process.exit(1);
    }
  })();
